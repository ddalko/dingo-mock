...
from regex_to_token_dfa import convert_to_token_level
...
def main():
    ...
    dataset = load_dataset('./json-mode-eval')['train']
    dataset = dataset.select(range(1))  # For testing, we only use 5 samples.

    for data in dataset:
        prompt = data['prompt']
        schema = data['schema']

        convert_to_token_level(schema, tokenizer)
        ...
        # out = generate(model, input_ids, attention_mask, temperature=0., cfg_scale=0., remasking='low_confidence')
